...
<body>
    ...
    <script>
        ...
        function refreshImage(imageEl, src) {
            if (!imageEl || !src) return;
            imageEl.src = src + "?t=" + Date.now();
        }

        function updatePreviewState(payload) {
            if (payload.original && originalPreview) {
                refreshImage(originalPreview, payload.original);
            }
            if (payload.processed && processedPreview) {
                refreshImage(processedPreview, payload.processed);
            }
            if (payload.status) {
        ...
        function schedulePreview() {
            ...
            previewTimer = setTimeout(async () => {
                ...
                try {
                    const payload = await postForm("/preview");
        ...
        fileInput.addEventListener("change", async () => {
            ...
            try {
                const payload = await postForm("/upload");
                updatePreviewState(payload);
            } catch (error) {
        ...
        resetButton.addEventListener("click", async () => {
            ...
            try {
                const payload = await postForm("/reset");
                updatePreviewState(payload);
                sliders.forEach((slider) => {
